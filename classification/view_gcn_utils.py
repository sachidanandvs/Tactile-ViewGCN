...
class View_selector(nn.Module):
    def __init__(self, n_views, sampled_view):
        ...
        self.cls = nn.Sequential(
            nn.Linear(128*self.s_views, 27*self.s_views),
            #nn.LeakyReLU(0.2),
            #nn.Linear(64*self.s_views,27*self.s_views))
        )
    def forward(self,F,vertices,k):
        ...
        F_score = self.cls(F1).reshape(F.shape[0],k,self.s_views,27).transpose(1,2)
        ...
        F1_id_F = F1_id.unsqueeze(-1).repeat(1, 1, 1, 128)
